...
const ContactForm = () => {
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      ...
      <button type="submit" disabled={loading} className="btn-gold w-full" style={{ padding: '14px', marginTop: '4px' }}>
  ...
  <section id="developer" className="py-10 sm:py-14 bg-[var(--color-bg-muted)] border-b border-gray-100">
    <div className="container mx-auto px-4 md:px-8">
      <div className="text-center mb-10" data-aos="fade-up">
        <span style={{ display: 'inline-block', padding: '4px 16px', background: 'var(--color-gold-bg)', borderRadius: '50px', fontSize: '11px', fontWeight: '700', color: GOLD, fontFamily: F_JOST, letterSpacing: '0.1em', textTransform: 'uppercase', border: '1px solid var(--color-gold-light)', marginBottom: '10px' }}>Developer Profile</span>
        <h2 style={{ fontFamily: F_JOST, fontWeight: '800', fontSize: '26px', color: PRIMARY, margin: '0 0 8px', letterSpacing: '-0.01em', textTransform: 'uppercase' }}>About the Developer</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-stretch max-w-6xl mx-auto">
        {/* Developer Info */}
        <div style={{ background: '#fff', display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 24px var(--color-shadow-card)', border: '1px solid var(--color-gold-light)' }} data-aos="fade-right" data-aos-delay="100">
          <div style={{ background: 'linear-gradient(135deg, var(--color-primary-dark), var(--color-primary))', padding: '18px 24px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', position: 'relative' }}>
              <div style={{ width: '42px', height: '42px', borderRadius: '10px', flexShrink: 0, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <h3 style={{ fontFamily: F_JOST, fontWeight: '800', fontSize: '18px', color: '#fff', margin: '0 0 4px', letterSpacing: '-0.01em' }}>Sobha Realty</h3>
            </div>
          </div>
          <div style={{ padding: '24px 28px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <p style={{ color: '#555', fontFamily: F_SANS, lineHeight: 1.85, fontSize: '14px', margin: '0 0 20px' }}>
              With over three decades of excellence, Sobha Realty is synonymous with precision engineering, superior quality, and architectural grandeur. Known for its backward integration model, every Sobha project is built in-house — from design to delivery — ensuring unmatched quality control and timely possession.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '22px' }}>
              {[
                { value: '30+', label: 'Years of Excellence' },
                { value: '130+', label: 'Projects Delivered' },
                { value: '40,000+', label: 'Happy Families' },
              ].map((stat, i) => (
                <div key={i} style={{ textAlign: 'center', padding: '14px 8px', background: 'var(--color-bg-muted)', borderRadius: '10px', border: '1px solid var(--color-gold-light)' }}>
            ...
            <button onClick={() => setIsOpen(true)} className="btn-gold" style={{ padding: '11px 28px', fontSize: '13px', width: '100%' }}>Know More</button>
          </div>
        </div>

        {/* Contact Form */}
        <div style={{ background: '#fff', borderRadius: '16px', boxShadow: '0 4px 24px var(--color-shadow-card)', border: '1px solid var(--color-gold-light)', overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%' }} data-aos="fade-left" data-aos-delay="200">
          <div style={{ background: 'linear-gradient(135deg, var(--color-primary-dark), var(--color-primary))', padding: '18px 24px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: 'linear-gradient(90deg, var(--color-gold), var(--color-gold-light))' }} />
            ...
            <p style={{ fontFamily: F_SANS, fontSize: '12px', color: 'rgba(255,255,255,0.6)', margin: 0 }}>Register now to get the best deals &amp; offers</p>
          </div>
