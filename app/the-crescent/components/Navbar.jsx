    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-[var(--color-bg)] transition-shadow duration-300 ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
      ...
      <div className="container mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between h-[80px]">
          ...
          <a href="#" className="flex items-center shrink-0">
            <img src="/images/logo/logo.webp" alt="Sobha Crescent" className="h-14 md:h-16 w-auto object-contain transition-all" style={{ maxWidth: '250px' }} />
          </a>
          ...
          <button onClick={() => setIsOpen(true)} className="hidden lg:flex btn-gold" style={{ borderRadius: '50px' }}>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-[var(--color-bg)] border-t border-gray-100 shadow-lg">
          ...
          <div className="p-4">
            <button onClick={() => { setIsOpen(true); setMobileOpen(false) }} className="w-full btn-gold" style={{ borderRadius: '50px' }}>
              Enquire Now →
            </button>
          </div>
        </div>
